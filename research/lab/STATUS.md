...
| [`model-comparison-harness`](https://github.com/Furkiozknn/model-comparison-harness) | Aynı isteği birden fazla backend'e paralel gönderip gecikme/başarı/sonuç karşılaştırması (`mch`) | ✅ main'e push edildi (başka bir Claude oturumu repoyu açtı), CI yeşil | ✅ 37/37 test, ai-job-gateway ile uçtan uca entegre, Reviewer denetiminden geçti |
| [`asset-provenance-toolkit`](https://github.com/Furkiozknn/asset-provenance-toolkit) | Üretilen dosyalara pipeline provenance (capability/provider/params/job id) gömme/çıkarma — PNG native + evrensel sidecar | ✅ main'e push edildi (başka bir Claude oturumu repoyu açtı), CI yeşil | ✅ 48/48 test, `ai-job-gateway`'e karşı uçtan uca doğrulandı (`from-job`) |
| [`ai-workflow-engine`](https://github.com/Furkiozknn/ai-workflow-engine) | DAG pipeline orkestratörü — YAML ile tanımlı, `ai-job-gateway` job'larını zincirliyor (generate→upscale→lipsync), bağımsız adımlar eşzamanlı | ✅ main'e push edildi (başka bir Claude oturumu repoyu açtı), CI yeşil | ✅ 34/34 test, `ai-job-gateway`'e karşı uçtan uca doğrulandı, Reviewer denetiminden geçti (1 HIGH bulgu bulundu ve düzeltildi: malformed Jinja2 syntax artık ham exception yerine temiz `PipelineRunError`) |
...
3. **`webhook-sink`** — `webhook-sink serve`: sıfır çalışma-zamanı bağımlılıklı (sadece `http.server`) yerel webhook test alıcısı, `--fail-first N` ile bir webhook göndericisinin retry mantığını gerçek bir 500 hatasına karşı test etmeyi sağlıyor. `ai-job-gateway`'in kendi webhook retry mantığı bugüne kadar sadece mock transport'larla test edilmişti. 14 test. Uçtan uca doğrulandı: gerçek çalışan bir `ai-job-gateway` sunucusuna `webhook_url` ile bir job submit edildi, tam job kaydının bu sink'e doğru şekilde ulaştığı doğrulandı.
